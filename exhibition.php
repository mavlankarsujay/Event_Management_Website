...
$time = $_POST['time'];
	...
	if (!empty($fname) || !empty($lname) || !empty($email) || !empty($phone) || !empty($category) || !empty($date) || !empty($time))
	{
		$host = "localhost:3307";
		$dbUsername = "root";
		$dbPassword = "";
		...
		if(mysqli_connect_error()){
			...
			$SELECT = "SELECT email from register_e Where email = ? Limit 1";
			$INSERT = "INSERT Into register_e (fname, lname, email, phone, category, date, time)
						values(?,?,?,?,?,?,?)";
			...
			if ($rnum==0){
				...
				$stmt->bind_param("sssisii",$fname, $lname, $email, $phone, $category, $date, $time);
				$stmt->execute();
				echo "";
			}
			else{
